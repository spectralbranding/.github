...
**The organizational caveat, and it is the counterpart of the one above:** the practitioner ladder is *causal* — each stage is supposed to produce the next, and the last stage is money. The specification cascade is a *traceability* relation — each level is **justified by** the level above it, and its terminal object is a checkable contract rather than a result. They are read in opposite directions, and only one of them has an instrument here. This work verifies the upward relation and has no executed evidence on the downward one. Laying the cascade over a strategy-to-execution ladder will be read as a performance claim; it is not one.
...
**The full accounting, marked stage by stage with the paper keys behind each verdict**, is published rather than summarised: the brand chain at [spectralbranding.com/coverage](https://spectralbranding.com/coverage), and the three organizational maps at [orgschema.com/coverage](https://orgschema.com/coverage). Both mark every stage as owned, touched, ceded or empty, with the evidence grade and the papers behind each verdict. The gaps those maps imply are listed at [spectralbranding.com/open-problems](https://spectralbranding.com/open-problems).
...
7. **Measure it** — the **PRISM instrument family** (`prism-b-brand` — PRISM-B (2026as) for brand perception, plus the pre-registered siblings PRISM-M (2026az, metamerism), PRISM-T (2026ba, version-floor drift), PRISM-C (2026bb, stated-vs-revealed choice), and PRISM-O (2026bd, organizational optimization depth)) and [the Brand Spectrometer](https://doi.org/10.5281/zenodo.20775963) (2026ax — the applied instrument built on PRISM: a reproducible pipeline that reads cohort-resolved 8-dimension perception from public artifacts under ground-truth-absence, with operator/artifact noise floors), and the applied cases `r10-dove-case-study`, `r17-brand-triangulation`.
